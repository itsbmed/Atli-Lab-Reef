<template>
  <section class="dose-calendar">
    ...
    <div ref="printContent" class="calendar-document">
      <div class="print-heading"><b>ATI · REEFING MADE SIMPLE</b><h1>Dosierungsplan</h1><p>{{ aquariumName }} · {{ reportNumber }} · {{ volume }} l netto</p><p>Start: {{ displayDate(start) }} · Einmaliger Korrekturkurs</p></div>
      ...
      <p class="print-footer">Tagesmengen nach der Zugabe abhaken. Nach dem Kurs Werte kontrollieren und den weiteren Bedarf anhand der Messung bestimmen. Alle Mengen beziehen sich auf das angegebene Netto-Wasservolumen.</p>
    </div>
  </section>
</template>
...
const props = defineProps({ items: { type: Array, required: true }, aquariumName: String, reportNumber: String, volume: Number, startDate: String })
...
function printPlan() {
  ...
  // Use the already rendered Vue content: user strings are escaped by Vue.
  popup.document.write(`<!doctype html><html lang="de"><head><meta charset="utf-8"><title>ATI Dosierungsplan</title><style>${printStyles} body{background:#fff}.calendar-day{min-height:90px}.calendar-week{margin:12px 0}.course-products{margin:12px 0}.print-heading{padding-bottom:12px}</style></head><body>${printContent.value.innerHTML}</body></html>`)
  popup.document.close()
  popup.opener = null
  popup.focus()
  popup.setTimeout(() => popup.print(), 250)
}
const printStyles = `@page{size:A4 landscape;margin:12mm}*{box-sizing:border-box}body{font:11px Arial,sans-serif;color:#0a1b43;margin:0}h1{font-size:30px;margin:8px 0}p{line-height:1.5}.print-heading{padding:0 0 18px;border-bottom:3px solid #0072ce}.print-heading b{color:#0072ce;letter-spacing:2px}.course-products{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:18px 0}.course-products article{padding:12px;border:1px solid #cbd9e7;border-radius:8px;break-inside:avoid}.course-products p{margin:6px 0}.course-products small{color:#425872}.calendar-week{margin:18px 0;break-inside:avoid}.calendar-week h4{margin:0 0 8px;font-size:14px;color:#0072ce}.calendar-days{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));border:1px solid #cbd9e7;border-radius:8px;overflow:hidden}.calendar-day{min-height:110px;padding:10px;border-right:1px solid #cbd9e7}.calendar-day:last-child{border:0}h5{font-size:12px;margin:0 0 12px}.calendar-dose{display:flex;gap:6px;padding:8px 0;border-top:1px solid #e4ebf2;break-inside:avoid}.calendar-dose div{min-width:0}.calendar-dose strong,.calendar-dose b,.calendar-dose small{display:block;overflow-wrap:anywhere}.calendar-dose b{color:#0072ce;margin:4px 0;font-size:13px}.calendar-dose small,.no-dose{color:#526780}.tick-box{width:10px;height:10px;border:1px solid #526780;flex:none;margin-top:2px}.print-footer{padding-top:14px;border-top:1px solid #cbd9e7;color:#526780;font-size:10px}`
</script>

<style scoped>
.dose-calendar{display:grid;gap:18px;min-width:0}.dose-calendar>header{display:flex;justify-content:space-between;align-items:center;gap:20px}.eyebrow{font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:var(--brand-blue);font-weight:700}.dose-calendar h3{font-size:24px;color:var(--brand-navy);margin:5px 0}.dose-calendar p{color:var(--text-muted);line-height:1.5;font-size:13px}.calendar-actions{display:flex;align-items:end;gap:12px}.calendar-actions label{display:grid;gap:6px;font-size:12px;font-weight:600}.calendar-actions input{padding:10px;border:1px solid var(--border);border-radius:12px;font:inherit;background:#fff;color:var(--text)}.calendar-note{padding:14px;border-radius:12px;background:#eef7ff}.calendar-document{display:grid;gap:18px;min-width:0}.print-heading{display:none}.course-products{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px}.course-products article{padding:15px;border:1px solid var(--border);border-radius:14px;background:#f8fbfe}.course-products strong{font-size:14px;color:var(--brand-navy)}.course-products small{font-size:12px;color:var(--text-muted)}.calendar-week h4{font-size:14px;color:var(--brand-navy);margin-bottom:10px}.calendar-days{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));border:1px solid var(--border);border-radius:14px;overflow:hidden}.calendar-day{padding:12px 9px;min-height:140px;border-right:1px solid var(--border);background:#fff}.calendar-day:last-child{border:0}.calendar-day h5{font-size:12px;color:var(--brand-navy);margin-bottom:12px}.calendar-dose{display:flex;gap:5px;padding:10px 0;border-top:1px solid var(--border)}.calendar-dose>div{min-width:0}.calendar-dose strong,.calendar-dose b,.calendar-dose small{display:block;overflow-wrap:anywhere}.calendar-dose strong{font-size:12px}.calendar-dose b{font-size:14px;color:var(--brand-blue);margin:4px 0}.calendar-dose small{font-size:10px;color:var(--text-muted)}.tick-box{width:10px;height:10px;border:1px solid var(--text-muted);flex:none;margin-top:2px}.print-footer{font-size:12px}.calendar-error{color:#b53a2e!important}@media(max-width:1000px){.calendar-days{grid-template-columns:repeat(2,minmax(0,1fr))}.calendar-day{border-bottom:1px solid var(--border)}.dose-calendar>header,.calendar-actions{align-items:stretch;flex-direction:column}}
</style>
